...
may use all or only some of these possible patterns as an input to \verb+EBMultiTest+. For example, if we were interested in first 4 Patterns only, we'd define:
<<>>=
Parti = PosParti[-5,]
Parti
@
